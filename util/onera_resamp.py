...
def resamp_and_save(in_res, out_res, im_dir, label_dir, output, curr_ind, im_size, split):
    ...
    im1 = Image.open(im_dir+'/pair/img1.png')
    im2 = Image.open(im_dir+'/pair/img2.png')
    label = Image.open(label_dir+'/cm/cm.png')
...
if __name__ == '__main__':
    ...
    im_size = 256
    im_dir = '/Users/jacobriviere/Desktop/Personal_Projects/Onera_satellite_cd_dataset_images'

    # TRAIN
    train_label_dir = '/Users/jacobriviere/Desktop/Personal_Projects/Onera_satellite_cd_dataset_train_labels'
    output_dir = '/Users/jacobriviere/Desktop/Personal_Projects/LEVIR_Format_Onera/train'
    out_res_m = [5, 5]
    curr_ind = 0
    for tr_inner in os.listdir(train_label_dir):
        ...
        if tr_inner[-4:] != '.txt' and tr_inner != '.DS_Store':
            print(tr_inner)
            new_curr_ind = resamp_and_save(in_res=resolution_dict[tr_inner], out_res=out_res_m, im_dir=im_dir+f'/{tr_inner}', label_dir=train_label_dir+f'/{tr_inner}', output=output_dir, curr_ind=curr_ind, im_size=im_size, split='train')
            curr_ind = new_curr_ind
    # VAL
    val_label_dir = '/Users/jacobriviere/Desktop/Personal_Projects/Onera_satellite_cd_dataset_val_labels'
    output_dir = '/Users/jacobriviere/Desktop/Personal_Projects/LEVIR_Format_Onera/val'
    out_res_m = [5, 5]
    curr_ind = 0
    for va_inner in os.listdir(val_label_dir):
        
        if va_inner[-4:] != '.txt' and va_inner != '.DS_Store':
            print(va_inner)
            new_curr_ind = resamp_and_save(in_res=resolution_dict[va_inner], out_res=out_res_m, im_dir=im_dir+f'/{va_inner}', label_dir=val_label_dir+f'/{va_inner}', output=output_dir, curr_ind=curr_ind, im_size=im_size, split='val')
            curr_ind = new_curr_ind

    # TEST
    test_label_dir = '/Users/jacobriviere/Desktop/Personal_Projects/Onera_satellite_cd_dataset_test_labels'
    output_dir = '/Users/jacobriviere/Desktop/Personal_Projects/LEVIR_Format_Onera/test'
    out_res_m = [5, 5]
    curr_ind = 0
    for te_inner in os.listdir(test_label_dir):
        
        if te_inner[-4:] != '.txt' and te_inner != '.DS_Store':
            print(te_inner)
            new_curr_ind = resamp_and_save(in_res=resolution_dict[te_inner], out_res=out_res_m, im_dir=im_dir+f'/{te_inner}', label_dir=test_label_dir+f'/{te_inner}', output=output_dir, curr_ind=curr_ind, im_size=im_size, split='test')
            curr_ind = new_curr_ind
