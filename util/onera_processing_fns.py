...
def distance_between_geojson(path):
    with open(path) as f:
        obj = json.load(f)
    points_list = obj['features'][0]['geometry']['coordinates'][0]
    print(points_list)
...
if __name__ == "__main__":
    if sys.argv[1] == 'once':
        ...
        image_arr_sh = np.array(Image.open(image_path)).shape
    ...
    for inner in os.listdir(data_dir):
        if inner[-4:] != '.txt' and inner != '.DS_Store':
            print(inner)
            coordinates_path = data_dir+'/'+inner+f'/{inner}.geojson'
            image_path = data_dir+'/'+inner+'/pair/img1.png'
